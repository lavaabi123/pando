<div class="d-flex flex-column flex-fill hp-100">
    <div class="d-flex justify-content-between align-items-center p-3 border-bottom position-relative zIndex-3">
        <div class="fs-16 fw-5"><?php echo e(__("Result")); ?></div>
        <div class="d-block d-lg-none">
            <div class="btn btn-icon btn-sm btn-light btn-hover-danger b-r-50 a-rotate closeAIResult">
                <i class="fa-light fa-xmark"></i>
            </div>
        </div>
    </div>

    <div class="d-flex flex-column flex-column-fluid overflow-y-auto p-3 fs-14 hp-100 position-relative">
        
        <?php if(!empty($result)): ?>

            <?php $__currentLoopData = $result; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <form class="actionForm" action="<?php echo e(url_app("captions/save")); ?>" method="POST">
                    <input type="text" class="d-none" name="name" value="AI Caption">
                    <input type="text" class="d-none" name="type" value="2">
                    <textarea name="content" class="d-none"><?php echo e($value); ?></textarea>
                
                    <div class="card border-gray-300 shadow-none mb-3">
                        <div class="card-body"><?php echo e($value); ?></div>
                        <div class="card-footer d-flex justify-content-end">
                            <button class="btn btn-sm btn-outline btn-info b-r-50"><?php echo e(__("Save to caption")); ?></button>
                        </div>
                    </div>
                </form>

            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

        <?php endif; ?>

    </div>
</div><?php /**PATH /home/royalinkdevelopm/public_html/pando.royalinkdevelopment.com/modules/AppAIContents/resources/views/result.blade.php ENDPATH**/ ?>