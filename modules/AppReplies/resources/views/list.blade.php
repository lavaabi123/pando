@if( $replies->Total() > 0 )
	...
	<div class="col-12 mb-2">
	    <label class="card rounded-5 border-gray-300 shadow-none" for="reply_{{ $value->id_secure }}">
			<div class="d-flex flex-row border-bottom border-gray-300">
				<div class="card-body d-flex gap-8 p-2 align-items-center">
					<div class="d-flex gap-8 ps-1">
						<input class="form-check-input checkbox-item" type="checkbox" name="id[]" value="{{ $value->id_secure }}" id="reply_{{ $value->id_secure }}">
					</div>
					...
					<div class="card-toolbar d-flex gap-16">
						<div class="btn-group position-static">
							...
							<ul class="dropdown-menu dropdown-menu-end border-1 border-gray-300 px-1 w-100 max-w-120 min-w-120">
								<li>
									<a class="dropdown-item px-2 p-t-2 p-b-2 rounded d-flex align-items-center gap-8 fw-5 fs-13 actionItem" href="{{ module_url("update") }}" data-id="{{ $value->id_secure }}" data-popup="replyModal" data-call-success="">
...
@else
	<div class="d-flex flex-column align-items-center justify-content-center py-5 my-5">
	    ...
	    <div class="text-body-secondary mb-4 text-center max-w-500">
	        {{ __('Start saving your favorite replies to reuse and streamline your content creation process.') }}
	    </div>
	    <a class="btn btn-dark actionItem" href="{{ module_url("update") }}" data-popup="replyModal" data-call-success="Main.ajaxScroll(true);">
	        <i class="fa-light fa-plus me-1"></i> {{ __('Add new reply') }}
	    </a>
	</div>
@endif