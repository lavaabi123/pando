...
namespace Modules\AppReplies\Http\Controllers;
...
class AppRepliesController extends Controller
{
    ...
    public function list(Request $request){
        ...
        $replies = $query->orderByDesc('changed')->paginate($per_page);

        if( $replies->Total() == 0 && $current_page > 1 ){
            ms([
                "status" => 0
            ]);
        }
    ...
    public function getReply(){
        ms([
            "status" => 1,
            "data" => view( module('key') .'::get_reply', [
            ])->render()
        ]);
    }
}
