...
function liprofile_renderItem(el) {
...
function liprofile_renderGrid(elements) {
    var total   = elements.length;
    var visible = elements.slice(0, 4);
    var more    = total - 4;
    if (total === 1) {
        return '<div class="cpv-grid" style="grid-template-columns:1fr;">' + liprofile_renderItem(elements[0]) + '</div>';
    } else if (total === 2) {
        return '<div class="cpv-grid" style="grid-template-columns:repeat(2,1fr);">'
             + visible.map(function(el){ return liprofile_renderItem(el); }).join('') + '</div>';
    } else if (total === 3) {
        return '<div class="cpv-grid" style="grid-template-columns:2fr 1fr;grid-template-rows:repeat(2,1fr);">'
             + '<div class="img-wrap" style="grid-row:span 2;">' + liprofile_renderItem(elements[0]) + '</div>'
             + liprofile_renderItem(elements[1]) + liprofile_renderItem(elements[2]) + '</div>';
    } else {
        var html = '<div class="cpv-grid" style="grid-template-columns:repeat(2,1fr);">';
        visible.forEach(function(el, idx) {
            var item = liprofile_renderItem(el);
            if (idx === 3 && more > 0) {
...
document.querySelectorAll('.cpv-linkedin-img').forEach(function(container) {
    ...
    function liprofile_getElements() {
    ...
    function liprofile_update() {
        var els = liprofile_getElements();
        if (els.length === 0) return;
        view.innerHTML = liprofile_renderGrid(els);
    }

    // Watch the cpv-img staging div (written by onMediaItemsChange)
    new MutationObserver(liprofile_update).observe(container, {
    ...
    if (stagingList) {
        new MutationObserver(liprofile_update).observe(stagingList, {
