...
namespace Modules\AppHashtags\Http\Controllers;
...
class AppHashtagsController extends Controller
{
    ...
    public function list(Request $request){
        ...
        $hashtags = $query->orderByDesc('changed')->paginate($per_page);

        if( $hashtags->Total() == 0 && $current_page > 1 ){
            ms([
                "status" => 0
            ]);
        }
    ...
    public function getHashtag(){
        ms([
            "status" => 1,
            "data" => view( module('key') .'::get_hashtag', [
            ])->render()
        ]);
    }
}
