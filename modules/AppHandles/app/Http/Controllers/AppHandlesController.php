...
namespace Modules\AppHandles\Http\Controllers;
...
class AppHandlesController extends Controller
{
    ...
    public function list(Request $request){
        ...
        $handles = $query->orderByDesc('changed')->paginate($per_page);

        if( $handles->Total() == 0 && $current_page > 1 ){
            ms([
                "status" => 0
            ]);
        }
    ...
    public function saveHandle(Request $request){
    ...
    public function getHandle(){
        ms([
            "status" => 1,
            "data" => view( module('key') .'::get_handle', [
            ])->render()
        ]);
    }
}
